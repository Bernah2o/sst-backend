...
def create_absenteeism(
    ...
    try:
        ...
        worker_check = db.execute(text("SELECT id FROM workers WHERE id = :worker_id"), 
                                 {"worker_id": absenteeism_data.worker_id}).fetchone()
        if not worker_check:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Trabajador no encontrado"
...
def get_absenteeism_list(
    ...
    try:
        ...
        if event_type:
            ...
            params["event_type"] = event_type.value if hasattr(event_type, 'value') else str(event_type)
        
        if event_month:
            where_conditions.append("a.event_month = :event_month")
            params["event_month"] = event_month.value if hasattr(event_month, 'value') else str(event_month)
        ...
        if search:
            where_conditions.append("(CONCAT(w.first_name, ' ', w.last_name) ILIKE :search OR w.document_number ILIKE :search)")
        ...
        for row in raw_items:
            ...
            item_dict = {
                ...
                "worker_name": f"{row.first_name or ''} {row.last_name or ''}".strip() if row.first_name or row.last_name else None,
...
def get_absenteeism(
    ...
    try:
        ...
        absenteeism_dict = {
            ...
            "worker_name": f"{row.first_name} {row.last_name}" if row.first_name and row.last_name else None,
            "worker_email": row.email,
            "worker_phone": row.phone,
            "cedula": row.cedula,
            "cargo": row.cargo,
...
def get_absenteeism_stats(
    ...
    try:
        ...
        if year:
            where_conditions.append("EXTRACT(year FROM start_date) = :year")
            params["year"] = year
        
        if worker_id:
            where_conditions.append("worker_id = :worker_id")
            params["worker_id"] = worker_id
        
        where_clause = " WHERE " + " AND ".join(where_conditions) if where_conditions else ""
        
        # Estadísticas básicas con SQL directo
        stats_query = f"""
        SELECT 
        ...
        for event_type in EventType:
            ...
            FROM absenteeism
            WHERE event_type = :event_type{' AND ' + ' AND '.join(where_conditions) if where_conditions else ''}
            """
        ...
        for month in EventMonth:
            ...
            SELECT COUNT(*) as count
            FROM absenteeism
            WHERE event_month = :event_month{' AND ' + ' AND '.join(where_conditions) if where_conditions else ''}
            """

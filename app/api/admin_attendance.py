...
from app.schemas.attendance import (
    ...
    AttendanceListResponse, BulkAttendanceCreate, BulkAttendanceResponse
)
...
async def get_all_attendance_records(
    ...
    attendance_records = query.order_by(Attendance.session_date.desc()).offset(skip).limit(limit).all()
        ...
        try:
            ...
            existing_attendance = db.query(Attendance).filter(
                and_(
                    ...
                    Attendance.course_id == bulk_data.course_id,
                    func.date(Attendance.session_date) == func.date(bulk_data.session_date)
                )
            ).first()
...
async def get_course_attendance_stats(
    ...
    partial_count = sum(1 for record in attendance_records if record.status == AttendanceStatus.PARTIAL)
    ...
    for record in attendance_records:
        if record.user_id not in user_stats:
            ...
            user_stats[record.user_id] = {
                "user_id": record.user_id,
                "name": f"{user.first_name} {user.last_name}" if user else "Usuario desconocido",
    ...
    for user_id, stats in user_stats.items():
        stats["attendance_rate"] = ((stats["present"] + stats["late"] + stats["partial"]) / stats["total_sessions"] * 100) if stats["total_sessions"] > 0 else 0

        ...
        try:
            ...
            if existing_enrollment:
                ...
                if (
                    ...
                    or existing_enrollment.notes != assignment_data.notes
                ):
                    existing_enrollment.status = assignment_data.status
...
async def bulk_assign_courses_to_workers(
    ...
    for (
        worker_id
    ) in assignment_data.user_ids:  # user_ids contains worker IDs in this context
        try:
...
async def delete_enrollment(
    ...
    worker = (
        db.query(Worker).filter(Worker.user_id == enrollment.user_id).first()
    ...
    try:
        ...
        orphaned_attendance_count = (
            db.query(Attendance)
            .filter(
                Attendance.user_id == enrollment.user_id,
                # Attendance.course_id == enrollment.course_id,  # Eliminado: ya no existe course_id
        ...
        material_progress_count = (
            ...
            .filter(UserMaterialProgress.enrollment_id == enrollment_id)
            .count()
        )
        ...
        module_progress_count = (
            ...
            .filter(UserModuleProgress.enrollment_id == enrollment_id)
            .count()
        )
        ...
        user_evaluations = (
            ...
            .filter(UserEvaluation.enrollment_id == enrollment_id)
            .all()
        )
        ...
        course_evaluations = (
            db.query(Evaluation)
            .filter(Evaluation.course_id == enrollment.course_id)
            .all()
        )
        ...
        for user_eval in all_evaluations:
            answers_count = (
                db.query(UserAnswer)
                .filter(UserAnswer.user_evaluation_id == user_eval.id)
                .count()
            )
        ...
        user_surveys = (
            db.query(UserSurvey).filter(UserSurvey.enrollment_id == enrollment_id).all()
        )
        ...
        for user_survey in user_surveys:
            answers_count = (
                ...
                .filter(UserSurveyAnswer.user_survey_id == user_survey.id)
                .count()
            )
...
async def get_course_workers(
    ...
    for enrollment in enrollments:
        ...
        worker_data = {
            ...
            "status": (
                enrollment.status
                if isinstance(enrollment.status, str)
                else enrollment.status.value
            ),
            "progress": enrollment.progress,
            "enrolled_at": (
...
async def remove_worker_from_course(
    ...
    worker_id: int,
    reason: str = Query(
        None, description="Justificación para retirar al trabajador del curso"
    ),
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db),
) -> Any:
    ...
    enrollment = (
        db.query(Enrollment)
        .filter(Enrollment.user_id == worker.user_id, Enrollment.course_id == course_id)
        .first()
    )
    ...
    try:
        ...
        attendance_records = (
            db.query(Attendance).filter(Attendance.enrollment_id == enrollment.id).all()
        )
        ...
        user_evaluations = (
            db.query(UserEvaluation)
            .filter(UserEvaluation.enrollment_id == enrollment.id)
            .all()
        )
        ...
        user_progress_records = (
            db.query(UserMaterialProgress)
            .filter(UserMaterialProgress.enrollment_id == enrollment.id)
            .all()
        )
        ...
        user_surveys = (
            db.query(UserSurvey).filter(UserSurvey.enrollment_id == enrollment.id).all()
        )
        ...
        enrollment_details.append(
            {
                ...
                "fecha_de_ingreso": (
                    user.hire_date.isoformat() if user and user.hire_date else None
                ),
                ...
                "updated_at": (
                    enrollment.updated_at.isoformat() if enrollment.updated_at else None
                ),
            }
        )
...
async def get_enrollment_stats(
    ...
    stats_query = (
        db.query(Enrollment.status, func.count(Enrollment.id).label("count"))
...
async def get_my_enrollments(
    course_id: Optional[int] = Query(
        None, description="Filter enrollments by course ID"
    ),
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db),
) -> Any:
    ...
    return {"items": enrollment_details, "total": len(enrollment_details)}
...
async def update_enrollment(
    ...
    if enrollment_data.status is not None:
        ...
        if (
            enrollment_data.status == EnrollmentStatus.ACTIVE

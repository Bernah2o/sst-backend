...
class InlineQuiz(Base):
    ...
    question_type = Column(SQLEnum(QuestionType, values_callable=lambda enum: [e.value for e in enum]), nullable=False)
...
class InteractiveActivity(Base):
    ...
    activity_type = Column(SQLEnum(ActivityType, values_callable=lambda enum: [e.value for e in enum]), nullable=False)
    order_index = Column(Integer, default=0)
